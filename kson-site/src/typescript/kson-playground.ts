...
import {
    HTMLEditorElement,
    createEditorConfig,
    safeAsyncCall, PLAYGROUND_EDITOR_CONFIG
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const preInputEditor = document.getElementById('input-editor') as HTMLEditorElement;
    const preOutputEditor = document.getElementById('output-editor') as HTMLEditorElement;
    ...
    const inputEditor = document.getElementById('input-editor') as HTMLEditorElement;
    const outputEditor = document.getElementById('output-editor') as HTMLEditorElement;
    const jsonRadio = document.getElementById('json-radio') as HTMLInputElement;
    const yamlRadio = document.getElementById('yaml-radio') as HTMLInputElement;
    ...
    async function createEditor(codeBlock: HTMLEditorElement, uri: string = '/workspace/playground.kson') {
        ...
        const sharedConfig = createEditorConfig(
            codeBlock,
            '',
            uri,
            PLAYGROUND_EDITOR_CONFIG
        );
        codeBlock._ksonEditor = uri.endsWith('.kson') ? 
            await createKsonEditor(sharedConfig) : await createYamlEditor(sharedConfig);
    }

    function updateEditorContent(editor: HTMLEditorElement, content: string) {
        if (!editor) return;
    ...
    async function updatePreview() {
        if (!inputEditor?._ksonEditor || !outputEditor?._ksonEditor) return;
    ...
    function clearPlaceholder(editor: HTMLEditorElement) {
